        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-12 border-b border-slate-100 pb-12">
            <div className="space-y-6">
                ...
                <div className="flex gap-4">
                    <button 
                        ...
                        className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${activeTeam === 'manchester' ? 'bg-[#660099] text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                    >
                        Manchester Team
                    </button>
                    <button 
                        onClick={() => setActiveTeam('shanghai')}
                        className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${activeTeam === 'shanghai' ? 'bg-[#660099] text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                    >
                        Shanghai Team
                    </button>
                </div>
            ...
            <div className="relative w-full md:w-80">
                ...
                <input 
                    ...
                    className="w-full bg-white border border-slate-200 rounded-2xl py-3 pl-12 pr-4 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-purple-100 shadow-sm"
                    value={searchTerm}
        ...
        {activeTeam === 'manchester' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredPeople.map((person, idx) => (
              <div key={idx} className="group bg-white border border-slate-100 rounded-3xl overflow-hidden hover:border-purple-200 transition-all flex flex-col shadow-md">
                 <div className="aspect-[4/5] relative overflow-hidden bg-slate-50">
                    ...
                    <div className="absolute top-4 right-4 flex gap-2">
                        <div className="p-2 bg-white/90 backdrop-blur rounded-xl text-[#660099] opacity-0 group-hover:opacity-100 transition-opacity shadow-sm">
                            <Mail size={14} />
                        </div>
                    </div>
                 </div>
          ...
          <div className="py-32 text-center space-y-6">
             <MapPin className="mx-auto text-[#ffcc00]" size={48} />
             <h3 className="text-2xl text-slate-900 serif italic">East China Normal University (ECNU)</h3>
